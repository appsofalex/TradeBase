...
struct ArchivedMessagesView: View {
    ...
    // Warning alert when tapping an archived conversation
    @State private var showUnarchiveWarning = false
    ...
    var body: some View {
        ...
        // Keep background NavigationLink for completeness, but we never set selectedConversation here anymore
        .background(
            NavigationLink(
                isActive: Binding(
        ...
        .alert("Conversation is archived", isPresented: $showUnarchiveWarning) {
            Button("OK", role: .cancel) { }
        } message: {
            Text("Unarchive this conversation to continue messaging.")
        }
    }
            ...
            if archived.isEmpty {
                ScrollView {
                    EmptyStateView(
                        ...
                        subtitle: "Swipe a conversation to archive it.",
                        icon: "archivebox"
                    )
                    .padding(.top, 60)
                }
                ...
                List {
                    ForEach(archived, id: \.id) { convo in
                        // Disable opening; show warning instead
                        Button {
                            showUnarchiveWarning = true
                        } label: {
                            ConversationRow(
                                conversation: convo,
                        ...
                        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                            ...
                            // Unarchive
                            if let me = state.currentAuthIdentity(), convo.isArchived(for: me) {
                                Button {
                                    Task { await toggleArchive(convo, archived: false) }
                                } label: {
    ...
    private func archivedConversations() -> [Conversation] {
